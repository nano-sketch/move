<script lang="ts">
    ...
    import { Play, RotateCcw, CheckCircle, XCircle, Loader2, CheckCircle2 } from "lucide-svelte";
    ...
    async function loadMonaco() {
      try {
        ...
        await new Promise((resolve, reject) => {
          ...
          setTimeout(() => reject(new Error("Monaco loading timeout")), 10000);
        });
    ...
    const programmingTerms: Record<string, string> = {
      ...
      parameter: "A variable used in a function to refer to one of the pieces of data provided as input",
    ...
    function createHighlightedText(text: string) {
      ...
      let lastIndex = 0;
      const termMatches: Array<{term: string, index: number, length: number}> = [];
  ...
  <svelte:head>
    ...
    <meta name="description" content="Solve coding challenge: {challenge?.title}" />
  </svelte:head>
  ...
  <div class="min-h-screen bg-background">
    ...
    <div class="mx-auto grid max-w-7xl grid-cols-2 gap-4 p-6 h-[calc(100vh-120px)]">
      <div class="flex flex-col overflow-hidden">
        <div class="rounded-lg border border-border bg-card overflow-hidden flex-1 flex flex-col">
          <div class="border-b border-border bg-muted/30">
            <div class="flex">
              <button onclick={() => activeTab = "description"} class="px-4 py-3 text-sm font-medium border-b-2 {activeTab === 'description' ? 'border-primary text-primary bg-card' : 'border-transparent text-muted-foreground hover:text-foreground'}">Description</button>
              <button onclick={() => activeTab = "testcases"} class="px-4 py-3 text-sm font-medium border-b-2 {activeTab === 'testcases' ? 'border-primary text-primary bg-card' : 'border-transparent text-muted-foreground hover:text-foreground'}">Test Cases</button>
            </div>
          </div>
          
          <div class="p-6 flex-1 overflow-y-auto" role="region" aria-label="Challenge description content" onmouseover={handleMouseEnter} onmouseout={handleMouseLeave} onfocus={handleMouseEnter} onblur={handleMouseLeave}>
            ...
            <div class="space-y-6">
              <div class="space-y-4">
                <div class="text-muted-foreground leading-relaxed">
                  {#each createHighlightedText(challenge?.detailed_description || challenge?.description || "") as part}
                    {#if part.isTerm}
                      <span class="programming-term cursor-help text-primary hover:text-primary/80 transition-colors duration-200 border-b border-dotted border-primary/30" role="button" tabindex="0" aria-label="Programming term: {part.text}. Hover for definition." data-term={part.term} onmouseenter={(e) => showTooltip(e, part.term || '')} onmouseleave={hideTooltip} onfocus={(e) => showTooltip(e, part.term || '')} onblur={hideTooltipImmediate}>{part.text}</span>
                    {:else}
                      {part.text}
                    {/if}
                  {/each}
              ...
              <div class="space-y-3">
                ...
                <div class="bg-muted/50 p-4 rounded-lg border">
                  <pre class="text-sm font-mono whitespace-pre-wrap text-foreground">{challenge.example}</pre>
                </div>
              </div>
              {/if}
            </div>
            ...
            <div class="space-y-4">
              ...
              <div class="space-y-3">
                ...
                <div class="flex items-start gap-3 rounded-lg border p-4 {result.passed ? 'border-green-500/20 bg-green-500/5' : 'border-red-500/20 bg-red-500/5'}">
                  {#if result.passed}
                  ...
                  <div class="flex-1 space-y-2">
                    <div class="font-mono text-sm text-muted-foreground"><span class="font-semibold">Input:</span> {JSON.stringify(result.input)}</div>
                    <div class="font-mono text-sm text-muted-foreground"><span class="font-semibold">Expected:</span> {JSON.stringify(result.expected)}</div>
                    <div class="font-mono text-sm {result.passed ? 'text-green-500' : 'text-red-500'}"><span class="font-semibold">Got:</span> {JSON.stringify(result.actual)}</div>
                  </div>
                </div>
                {/each}
              </div>
              {:else}
              <div class="text-center py-8 text-muted-foreground"><p>run code to see test results here</p></div>
              {/if}
            </div>
            {/if}
          </div>
      ...
      <div class="flex flex-col space-y-4 overflow-hidden">
        <div class="rounded-lg border border-border bg-card overflow-hidden flex-1 flex flex-col">
          <div class="border-b border-border bg-muted/30">
            <div class="flex items-center justify-between px-4 py-3">
              ...
              <div class="flex items-center gap-2">
                <button onclick={downloadCode} class="flex h-8 w-8 items-center justify-center rounded-md hover:bg-muted transition-colors" title="Download Code">
                  <svg class="h-4 w-4 text-muted-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                </button>
                <button onclick={resetCode} class="flex h-8 w-8 items-center justify-center rounded-md hover:bg-muted transition-colors" title="Reset Code">
                  <RotateCcw class="h-4 w-4 text-muted-foreground" />
                </button>
                <button onclick={runCode} disabled={is_running} class="flex items-center gap-2 px-4 py-2 bg-primary/10 hover:bg-primary/20 text-primary border border-primary/20 hover:border-primary/30 rounded-lg text-sm font-medium transition-all duration-75 ease-out disabled:opacity-50 disabled:cursor-not-allowed backdrop-blur-sm">
                  {#if is_running}
          ...
          <div bind:this={editorContainer} class="flex-1 w-full bg-[#1e1e1e]"></div>
        ...
        {#if output}
        <div class="rounded-lg border border-border bg-card max-h-32 overflow-y-auto">
          <div class="border-b border-border px-4 py-2 bg-muted/50">
            <span class="text-sm font-medium">Output</span>
          </div>
          <div class="p-4 font-mono text-sm text-muted-foreground">{output}</div>
        </div>
        {/if}
        ...
        <div class="rounded-lg border border-green-500/20 bg-green-500/10 p-4">
          <div class="flex items-center gap-2 justify-center text-green-500">
            <CheckCircle2 class="h-5 w-5" />
            <span class="font-medium">Challenge completed! Great work!</span>
          </div>
        </div>
        {/if}
      </div>
  ...
  {#if tooltip.show}
  <div class="fixed z-50 px-3 py-2 text-sm bg-popover border border-border rounded-md shadow-lg max-w-xs pointer-events-none" style="left: {tooltip.x}px; top: {tooltip.y}px; transform: translateX(-50%);">
    {tooltip.text}
