<script lang="ts">
    ...
    function toggleDebugMode() {
        debugMode = !debugMode;
        debugAttempts = 0;
        debugOutput = debugMode ? "debug mode enabled - unlimited tries but strict monitoring" : "";
        consoleOutput = "";
        resetChallenge();
    }
    function simulatePythonExecution(code: string): string {
        try {
            const currentError = challenges[currentChallenge].error;
            if (code.includes(challenges[currentChallenge].code.split('\n')[0])) {
                if (currentError.includes("TypeError")) {
                    return `traceback (most recent call last):
  file "<stdin>", line 6, in <module>
  file "<stdin>", line 3, in calculate_area
typeerror: can't multiply sequence by non-int of type 'str'`;
                } else if (currentError.includes("IndexError")) {
                    return `traceback (most recent call last):
  file "<stdin>", line 4, in <module>
indexerror: list index out of range`;
                } else if (currentError.includes("ZeroDivisionError")) {
                    return `traceback (most recent call last):
  file "<stdin>", line 6, in <module>
  file "<stdin>", line 2, in divide_numbers
zerodivisionerror: division by zero`;
                } else if (currentError.includes("KeyError")) {
                    return `Traceback (most recent call last):
  File "<stdin>", line 3, in <module>
KeyError: 'David'`;
                } else if (currentError.includes("AttributeError")) {
                    return `Traceback (most recent call last):
  File "<stdin>", line 2, in <module>
AttributeError: 'NoneType' object has no attribute 'upper'`;
                } else if (currentError.includes("ValueError")) {
                    return `Traceback (most recent call last):
  File "<stdin>", line 2, in <module>
ValueError: math domain error`;
                } else if (currentError.includes("RecursionError")) {
                    return `Traceback (most recent call last):
...
RecursionError: maximum recursion depth exceeded`;
            ...
            if (code.trim() === challenges[currentChallenge].solution.trim()) {
...
${currentError}`;
            ...
            return `simulation error: ${(error as Error).message}`;
        }
    }
    ...
    async function checkSolution() {
        ...
        if (userCode === correctSolution) {
            ...
            if (debugMode) {
                ...
                alert(`correct! challenge completed! total solutions: ${hallSolutions}`);
                resetChallenge();
            }
        } else {
            if (debugMode) {
        ...
        if (browser) {
            ...
            blurHandler = () => {
                if (!isLocked && timerStarted) {
                    const timeoutId = setTimeout(() => {
                        if ((document.hidden || !document.hasFocus()) && timerStarted) {
                            if (browser) localStorage.setItem('hallLeftPage', 'true');
                            focusLost = true;
                            resetChallenge();
                        }
...
<main class="relative min-h-screen bg-background select-none">
    <div class="absolute inset-0 bg-gradient-to-br from-background via-background/95 to-primary/5">
        <div class="absolute inset-0 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-primary/20 via-background/50 to-background"></div>
        ...
        <div class="max-w-7xl mx-auto">
            <div class="mb-6">
                <div class="flex items-center justify-between mb-4">
                    <div>
                        ...
                        <p class="text-sm text-muted-foreground">fix python errors under pressure</p>
                    </div>
                    <div class="flex items-center gap-4">
                        <div class="flex items-center gap-2 text-sm text-muted-foreground">
                            <span>challenge #{currentChallenge + 1}</span>
                            <span>•</span>
                            <span>attempts: {debugMode ? debugAttempts : attempts}</span>
                            <span>•</span>
                            ...
                            {#if timerStarted}
                                <span>•</span>
                                <span>timer: {Math.floor(timeLeft / 60)}:{(timeLeft % 60).toString().padStart(2, '0')}</span>
                                <span>•</span>
                            {/if}
                            <button onclick={exitChallenge} class="text-destructive hover:text-destructive/80 transition-colors">
                                exit challenge
                            </button>
                        </div>
                    </div>
            ...
            <div class="grid gap-6 lg:grid-cols-4">
                <div class="lg:col-span-3">
                    <div class="bg-card/80 backdrop-blur-sm border border-border rounded-lg overflow-hidden">
                        <div class="bg-muted/50 px-4 py-2 border-b border-border">
                            <div class="flex items-center justify-between">
                                <span class="text-sm font-medium">python editor</span>
                            </div>
                        </div>
                        <div 
                            bind:this={editorContainer} 
                            class="h-80 {isLocked ? 'pointer-events-none opacity-50' : ''}"
                        ></div>
                        <div class="border-t border-border">
                            <div class="bg-muted/50 px-4 py-2 border-b border-border">
                                <div class="flex items-center justify-between">
                                    ...
                                    <div class="flex items-center gap-2">
                                        <button 
                                            onclick={runCode}
                                            disabled={isRunning || (isLocked && !debugMode)}
                            ...
                            <div class="h-32 p-4 bg-muted/20 border border-border/50 mx-4 mb-4 rounded-md overflow-y-auto">
                                {#if consoleOutput}
                                    <pre class="text-xs font-mono text-foreground whitespace-pre-wrap leading-relaxed">{consoleOutput}</pre>
                                {:else}
                                    <span class="text-xs text-muted-foreground font-mono">click "run code" to see output and errors...</span>
                                {/if}
                            </div>
                        </div>
                        {#if debugMode && debugOutput}
                            <div class="p-4 border-t border-border">
                                <span class="text-xs text-muted-foreground">{debugOutput}</span>
                            </div>
                        {/if}
                    </div>
                </div>
                ...
                <div class="space-y-6">
                    <div class="bg-card/80 backdrop-blur-sm border border-border rounded-lg p-6">
                        ...
                        <ul class="text-sm text-muted-foreground space-y-2">
                            ...
                            <li class="text-destructive">• breaking rules resets challenge</li>
                            {#if debugMode}
                                <li class="text-primary">• debug: unlimited tries enabled</li>
                            {/if}
                        </ul>
                    </div>
                    ...
                    {#if showHint}
                        <div class="bg-card/80 backdrop-blur-sm border border-border rounded-lg p-6">
                            <h3 class="font-semibold mb-3 text-primary">hint</h3>
                    ...
                    <div class="bg-card/80 backdrop-blur-sm border border-border rounded-lg p-6">
                        <h3 class="font-semibold mb-3">{debugMode ? 'debug monitor' : 'focus mode'}</h3>
                        <p class="text-sm text-muted-foreground mb-3">
                            {debugMode ? 'unlimited tries but all cheating attempts are detected and reset the challenge.' : 'stay focused to solve the challenge. any distraction will reset your progress.'}
                        </p>
                        ...
                        {#if debugMode && debugOutput}
                            <div class="mt-3 p-2 bg-muted/50 rounded text-xs">
                                <strong>debug output:</strong> {debugOutput}
                            </div>
                        {/if}
                    </div>
                </div>
...
<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
    <div class="relative mx-4 w-full max-w-md bg-card border border-border/30 rounded-lg shadow-lg select-none">
        <div class="p-6">
            <button onclick={closeInstructionModal} class="absolute right-4 top-4 text-muted-foreground hover:text-foreground transition-colors" aria-label="Close modal">
                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"></path>
                </svg>
            </button>
            <div class="mb-6">
                <h2 class="text-xl font-medium text-foreground mb-2">hall of focus</h2>
                <p class="text-sm text-muted-foreground">debug python errors under time pressure</p>
            </div>
            <div class="mb-6">
                <p class="text-sm text-muted-foreground leading-relaxed mb-4">
                    a stress test environment with broken python code. fix all errors to pass each challenge.
                </p>
                <div class="grid grid-cols-2 gap-4 text-sm">
                    <div>
                        <h3 class="font-medium text-foreground mb-2">rules</h3>
                    ...
                    <div>
                        <h3 class="font-medium text-foreground mb-2">tips</h3>
                        <ul class="text-muted-foreground space-y-1 text-sm">
            ...
            <div class="flex gap-3">
                <button onclick={closeInstructionModal} class="flex-1 px-4 py-2 text-sm text-muted-foreground border border-border rounded-md hover:bg-muted/50 transition-none">
                    cancel
                </button>
                <button onclick={startHall} class="flex-1 px-4 py-2 text-sm text-white bg-orange-600 hover:bg-orange-700 rounded-md transition-none">
                    start challenge
                </button>
            </div>
        </div>
