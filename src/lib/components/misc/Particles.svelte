<script lang="ts">
    ...
    function resizeCanvas() {
        ...
        circles = []
        canvasSize = { w: canvasContainerRef.offsetWidth, h: canvasContainerRef.offsetHeight }
    ...
    let animationId: number, lastTime = 0
    const targetFPS = 30, frameInterval = 1000 / targetFPS
    
    function animate(t = 0) {
        if (t - lastTime < frameInterval) return animationId = requestAnimationFrame(animate)
        lastTime = t
        clearContext()
        circles.forEach((c, i) => {
            const edge = [c.x + c.translateX - c.size, canvasSize.w - c.x - c.translateX - c.size, c.y + c.translateY - c.size, canvasSize.h - c.y - c.translateY - c.size]
